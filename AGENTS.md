...
- `design/` 配下にあるデザインが正なので画面実装時はデザインが同一となるように常に参照して正確に反映する。
...
- 202509_StickyBoard/sticky_board/resources/js/components/ui：共通のuiコンポーネントがあれば必要に応じてこちらに作成する
- 202509_StickyBoard/sticky_board/resources/js/hooks：共通のロジックがあれば必要に応じてこちらに作成する。
...
- 基本は`202503_twitter/**`が手本になるが、そこにも見本となる実装がなかった場合は laravel-boost を利用し、ベストな実装を実施する
...
- ホストに環境はなくdocker環境を用いているため、何かしらのコマンドをたたく必要がある場合は、docker環境に入って作業する。
- エラー等で不具合が発生している場合は、laravel.logを参照する等で原因を探る
