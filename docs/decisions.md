...
- 日付: 2026-01-27
- 決定事項: Google 再ログイン時は、`/storage/avatars/` 配下のカスタムアバターが設定されている場合は Google のアバターで上書きしない。
- 理由: プロフィール編集で変更したアイコンがログインのたびに初期アイコンへ戻るのを防ぐため。
- 影響範囲: Google ログイン処理（`LoginController`）。
- 代替案: Google アバターを別カラムで保持して比較する。
